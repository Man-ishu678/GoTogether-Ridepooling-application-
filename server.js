...
// GET rides (optional pickup/dest filtering)
...
app.post('/api/bookings', async (req, res) => {
  try {
    ...
    if (!rideId || !riderName) return res.status(400).json({ error: 'Missing required fields (rideId, riderName)' });

    const ride = await Ride.findById(rideId);
    if (!ride) return res.status(404).json({ error: 'Ride not found' });
    if (ride.seatsAvailable <= 0) return res.status(400).json({ error: 'No seats available' });

    // decrement seats and save booking
...
// GET bookings (with ride populated)
